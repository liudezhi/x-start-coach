...
- 当前环境具备图片生成能力时，默认生成一张教学图片，直观呈现概念、关系、机制或架构。
...
对于完成的概念讲解、对比或教程，只要环境具备图片生成能力，就会默认生成一张教学图片，除非用户明确要求不生成：

- Codex：使用可用的 GPT Image 2 图片生成能力。
- Gemini CLI：安装了相关能力或扩展时使用 Nano Banana。
- Claude Code：使用已经配置的图片生成 MCP、插件、工具或 Skill。
- 其他兼容代理：使用对应的原生或已配置图片生成能力。

如果没有图片生成能力，则自动降级为 Mermaid 或文本关系图。
